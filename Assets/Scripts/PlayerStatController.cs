...
public class PlayerStatController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Apply purchased stats so the player starts with upgrades
        ApplyPurchasedStats();
    }
    ...
    public void ApplyPurchasedStats()
    {
        if (PlayerController.instance != null)
        {
            int msLevel = Mathf.Clamp(moveSpeedLevel, 0, moveSpeed.Count - 1);
            int prLevel = Mathf.Clamp(pickupRangeLevel, 0, pickupRange.Count - 1);
            int mwLevel = Mathf.Clamp(maxWeaponsLevel, 0, maxWeapons.Count - 1);

            PlayerController.instance.moveSpeed = moveSpeed[msLevel].value;
            PlayerController.instance.pickupRange = pickupRange[prLevel].value;
            PlayerController.instance.maxWeapons = Mathf.RoundToInt(maxWeapons[mwLevel].value);
        }

        if (PlayerHealthController.instance != null)
        {
            int hLevel = Mathf.Clamp(healthLevel, 0, health.Count - 1);
            PlayerHealthController.instance.maxHealth = health[hLevel].value;
